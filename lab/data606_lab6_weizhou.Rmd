...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
The sampling method should be completely random and the sample should be representative of all population. 
It doesn't seem like a reasonable assumption.
...
Each row of atheism corresponds to a respondent.    
...
```
The calculated percent of atheist is 5%, matches Table 6. 
...
2.Success-failure condition: Met    
...
```{r}
margin_error = (0.0634 - 0.0364) / 2 
margin_error
```

Standard error is 0.0069     
Margin of error for 95% confidence level is 0.0135
...
```{r}
n1=unique(atheism$nationality)[10]


n1
n112 =  subset(atheism, nationality == n1 & year == "2012")
inference(n112$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")

ME112 = ( 0.0273 - 0.0105 )/2
```

Margin of error is `r ME112` 


```{r}

n2 = unique(atheism$nationality)[12]
n2
n212 =  subset(atheism, nationality == n2 & year == "2012")
inference(n212$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")

n2=unique(atheism$nationality)[12]
ME212 = ( 0.0273 - 0.0105 )/2
```

Margin of error is `r ME212` 
...
When p get closer to 50%, the margin of error get larger.      
When p get closer to 0% or 100%, the margin of error get smaller.     
...
1. fits under the normal curve    
2. Unimodal and unskewwed    
3. The median and mean of the distribution are near around 0.1 
...
```{r}
par(mfrow = c(2, 2))
p_ls <- c(0.1,0.1,0.02,0.02)
n_ls <- c(1040,400,1040,400)
p_hats <- rep(0, 5000)
...
P=0.1,n=1040: Distribution centered at 0.1, unimodal, unskewwed narrower   
P=0.1,n=400: Distribution centered at 0.1, unimodal, unskewwed wider    
P=0.02,n=1040: Distribution centered at 0.02, unimodal, unskewwed narrower     
P=0.02,n=400: Distribution centered at 0.02, unimodal, unskewwed wider       
 
n changes the distribution spread    
p changes the distribution center  
...
Australia is sensible to proceed with inference and report margin of errors      
a. sample size n of 1039 represents less than 10% of total population of Australia.     
b. Proportion value p̂  = 0.1. Success = 104 and Failure = 935, both values are greater than 1    

Ecuador is not sensible toproceed with inference and report margin of errors as it matches only one condition.     
a. sample size n of 404 represents less than 10% of total population of Ecuador.       
b. Proportion value p̂  = 0.02. Success = 8 and Failure = 396, as success is less than 10 it fails success-failure condition.     
...
Since p−value=0.3966 is greater than 0.05, there is no convincing evidence that atheism index is different between years 2005 and 2012.
```{r}
spain <- atheism%>%
...
Since p−value=0  is less than 0.05, there is convincing evidence that atheism index has changed between years 2005 and 2012.
```{r}
us <- atheism%>%
...
We expected a change in 'r 39*0.05' countries. 
...
n           
```
For 95% confidence interval, we get the sample size as `r n`.
