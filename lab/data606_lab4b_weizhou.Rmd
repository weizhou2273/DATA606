...
hist(samp)
```
##### The distribution is unimodal, with a right skew. The typical size should be the average which is `r mean(samp)` 


2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

Another student's distribution should be similar but not identical since the sample is randomly acquired from population. 
...
The sample size must greater than 30    
IID (independent: <10% of the population)       
Not strongly skewed.  
...
We have 95% confident that the population mean falls within that interval. 
...
The confidence interval capture the true average size of houses in Ames
...
95% of those intervals would capture the true population mean, since the interval is calculated based on 95% confidence. 
...
p = mean(ifelse((lower_vector<=mean & mean<=upper_vector),1, 0))
```
`r p` of the confidence intervals include the true population mean `r mean`
...
```{r}
lower_vector <- samp_mean - cn * samp_sd / sqrt(n) 
upper_vector <- samp_mean + cn * samp_sd / sqrt(n)
plot_ci(lower_vector, upper_vector, mean(population))
```
