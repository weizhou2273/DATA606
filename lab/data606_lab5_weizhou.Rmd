...
There are 1000 cases in sample.
```{r}
nrow(nc)
```
...
```{r}
bp <-ggplot(nc, aes(x=habit, y = weight)) + geom_boxplot()
bp
```
...
`H0` There is no difference in the mean of the weights between the two population.    
`HA` There is a difference in the mean of the weights between the two population. 
...
At the 95% confidence level, the mean pregnancy length in weeks of population between 38.1528 and 38.5165. 
```{r}
...
At the 95% confidence level, the mean pregnancy length in weeks of population between 38.182 and 38.4873 
```{r}
inference(y = nc$weeks, est = "mean", type = "ci", null = 0, 
...
`H0` There is no difference in the mean of the weights gains between the younger and mature mothers.    
`HA` There is a difference in the mean of the weights gains between the younger and mature mothers. 
...
Since the p-value is 0.1686, which is greater than the significant level of 0.05, 
we fail to reject the null hypothesis that younger mothers and mature mothers have the same mean weight gain. 
...
```
By looking at the max age of younger mon and min age of mature mom, we can figure out the cutoff age should be 34/35.
...
Research question: Whether the average weight difference between male baby and female baby. 

`H0` There is no difference between male baby weight and female baby weight
`HA` There is a difference between male baby weight and female baby weight
```{r}
ggplot(nc, aes(x=gender, y=weight)) + geom_boxplot()
```

```{r}
inference(y = nc$weight, x = nc$gender, est = "mean", type = "ht", null = 0, alternative = "twosided", 
          method = "theoretical")
```

Since the p-value is 0, which is less than the significant level of 0.05, 
we can admit that there is significant difference between male baby weight and female baby weight at the significant level of 0.05. 
