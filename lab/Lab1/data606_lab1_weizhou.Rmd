...
summary(cdc$height)
```
The interquartile range of height is 64 to 70
```{r}
summary(cdc$age)
```
The interquartile range of age is 31 to 57
```{r}
table(cdc$gender=='m')
```
...
```
The relative frequency of gender is M:0.47845 F:0.52155
```{r}
prop.table((table(cdc$exerany)))
```
The relative frequency of Exerany is Yes:0.7457 F:0.2543
```{r}
prop.table(table((cdc$genhlth=='excellent')))
```
23.285% of the sample is in excellent health. 
...
Larger propotion of male have smoking habits comparing with women. 
...
```{r}
cdc$under23_and_smoke <-(cdc$age<23)&(cdc$smoke100==1)
head(cdc$under23_and_smoke)
```
...
The better the health condition, the lower the BMI. 

```{r}
bmi <- (cdc$weight / cdc$height^2) * 703
boxplot(bmi ~ cdc$exerany)
```
I choose exerany. I used boxplot. Usually exercise would lead to lower BMI.   
...
```
The Weight is positively correlated with the desired weight. 
...
```
'wdiff' is integer.   
If an observation 'wdiff' is 0 , that means the person's weight equal to desired weight.   
If 'wdiff' is positive, that means desired weight is greater than weight.   
If 'wdiff' is negative, that means desired weight is lower than weight.   
...
```{r}
hist(cdc$wdiff,breaks=100)
```
```{r}
summary(cdc$wdiff)
```
```{r}
barplot(table(cdc$wdiff>0))
```

According the summary and histgram: 
1. The center (median) of 'wdiff' distribution is 10
2. The shape is left skewed.   
3. The spread of weight difference is from -400 to 200.  
4. Most people believe their weight are greater than desired weight. 
...
```{r}
gender_wdiff <- table(cdc$wdiff >0 , cdc$gender)
gender_wdiff
```
```{r}
boxplot(cdc$wdiff ~ cdc$gender)
```
According to boxplot, women tends to believe their current weight is greater than desired weight than men.   
...
lower <- mean-sd
upper <- mean+sd
prop.table(table((cdc$weight<upper) & (cdc$weight>lower)))
```
70.76% of records are within one standard deviation of the mean. 
