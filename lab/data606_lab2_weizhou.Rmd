...
<span style="color:blue">One hit and one miss in a streak of 1. Streak length of 0 only has one miss.</span>
...
<span style="color:blue">The distribution is right skewed. The typical streak length is 0. The longest streak is 4</span>
## Compared to What?
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    came up heads? <span style="color:blue">20 flips come up heads</span>
```{r}
...
`r max(sim_streak)` is the longest streak of baskets in 133 shots.
</span>
...
```{r}
sim_basket_2 <- sample(outcomes, size = 133, replace = TRUE,prob = c(0.45, 0.55))
sim_streak_2<- calc_streak(sim_basket_2)
barplot(table(sim_streak_2))
```
```{r}
prop.table(table(sim_streak))
prop.table(table(sim_streak_2))

```

<span style="color:blue">
We should expect something similar.Since the number of shots and the possibility of hits have been defined, the result should be similar. 
Given the law of large numbers, if the number of shots is much larger than 133, we should expect an even closer result.  
</span>
...
<span style="color:blue"> Barplot comparison</span>
```{r}
barplot(table(kobe_streak))
barplot(table(sim_streak))
```
...
<span style="color:blue"> Summary comparison</span>
```{r}
summary(kobe_streak)
summary(sim_streak)
```
<span style="color:blue"> Proportion comparison</span>
```{r}
...
<span style="color:blue"> 
Simlulation streak length is higher than Kobe's streak length. (`r max(sim_streak)` v.s 4)    
By comparing the Kobe's performance with the simulation result, it is hard to say the model fits Kobe's pattern:   
1. Streak Length 1 has larger proportion in Kobe's pattern but relative lower in the model.   
2. Longest streak length in Model is longer and have larger proportion than Kobe's pattern.    
The overall similation is outperformed than Kobe's pattern. In real workd basket match, a lot more factors will affect Kobe's performance.   
</span>
