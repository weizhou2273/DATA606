...
```{r}
library(ggplot2)
ggplot(bdims, aes(x=hgt,color=sex))+
...
```
Based on the plot, it appears that the data follows a nearly normal distribution.
...
The normal probability plot for femail heights looks similar to the simulated data plots.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
The normal probability plot for femail weights looks similar to the simulated data plots.
```{r}
qqnormsim(fdims$wgt)
```
...
What is the probability that a randomly chosen female is taller than 150cm?
...
* * *
What is the probability that a randomly chosen female weight is greater than 80 kg?
```{r}
...
```
Height had a closer agreement between the two methods
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter ____. B

    **b.** The histogram for female elbow diameter (`elb.di`) belongs to normal 
    probability plot letter ____. C
